...
//Nullam volutpat ac risus in fermentum.`
    ...
    if charsheet0 {
        ...
        text.Draw(screen, fmt.Sprintf("Str: %+d", strstats[1]), fo, 32, 96, color.White)
        text.Draw(screen, fmt.Sprintf("Dex: %+d", dexstats[1]), fo, 32, 128, color.White)
        text.Draw(screen, fmt.Sprintf("Con: %+d", constats[1]), fo, 32, 160, color.White)
        text.Draw(screen, fmt.Sprintf("Int: %+d", intelstats[1]), fo, 32, 192, color.White)
        text.Draw(screen, fmt.Sprintf("Wis: %+d", wisstats[1]), fo, 32, 224, color.White)
        text.Draw(screen, fmt.Sprintf("Cha: %+d", chastats[1]), fo, 32, 256, color.White)
        text.Draw(screen, "Skills:", fo, 256, 64, color.White)
        text.Draw(screen, fmt.Sprintf("Acrobatics:      %+d", dexstats[1]), fo, 256, 96, color.White)
        text.Draw(screen, fmt.Sprintf("Animal Handling: %+d", wisstats[1]), fo, 256, 128, color.White)
        text.Draw(screen, fmt.Sprintf("Arcana:          %+d", intelstats[1]), fo, 256, 160, color.White)
        text.Draw(screen, fmt.Sprintf("Athletics:       %+d", strstats[1]), fo, 256, 192, color.White)
        text.Draw(screen, fmt.Sprintf("Deception:       %+d", chastats[1]), fo, 256, 224, color.White)
        text.Draw(screen, fmt.Sprintf("History:         %+d", intelstats[1]), fo, 256, 256, color.White)
        text.Draw(screen, fmt.Sprintf("Insight:         %+d", wisstats[1]), fo, 256, 288, color.White)
        text.Draw(screen, fmt.Sprintf("Intimidation:    %+d", chastats[1]), fo, 256, 320, color.White)
        text.Draw(screen, fmt.Sprintf("Investigation:   %+d", intelstats[1]), fo, 256, 352, color.White)
        text.Draw(screen, fmt.Sprintf("Medicine:        %+d", wisstats[1]), fo, 512, 96, color.White)
        text.Draw(screen, fmt.Sprintf("Nature:          %+d", intelstats[1]), fo, 512, 128, color.White)
        text.Draw(screen, fmt.Sprintf("Perception:      %+d", wisstats[1]), fo, 512, 160, color.White)
        text.Draw(screen, fmt.Sprintf("Performance:     %+d", chastats[1]), fo, 512, 192, color.White)
        text.Draw(screen, fmt.Sprintf("Persuasion:      %+d", chastats[1]), fo, 512, 224, color.White)
        text.Draw(screen, fmt.Sprintf("Religion:        %+d", intelstats[1]), fo, 512, 256, color.White)
        text.Draw(screen, fmt.Sprintf("Sleight of Hand: %+d", dexstats[1]), fo, 512, 288, color.White)
        text.Draw(screen, fmt.Sprintf("Stealth:         %+d", dexstats[1]), fo, 512, 320, color.White)
        text.Draw(screen, fmt.Sprintf("Survival:        %+d", wisstats[1]), fo, 512, 352, color.White)
        text.Draw(screen, ">", fo, 704, 512, color.White)
    } else if charsheet2 {
        screen.DrawImage(blankImage, nil)
        text.Draw(screen, "<", fo, 64, 512, color.White)
